...
description: Notion presents itself as the all-in-one workspace through a confident, illustration-rich brand voice — anchored by a deep navy hero band ({colors.brand-navy}) decorated with brand-colored sticky-note dots and mesh wire illustrations, a signature blue pill primary CTA ({colors.primary}), and a rich palette of pastel-tinted feature cards that echo the colorful database properties of the live product. The system uses a Notion-Sans (Inter-based) typeface across every UI surface, anchors a 4-tier pricing comparison (Free / Plus / Business / Enterprise), and presents the live workspace UI mockup directly inside the hero band. Coverage spans homepage, Enterprise, Product AI, Product Agents, Startups, and Pricing surfaces.

colors:
  primary: "#1e73d8"
  primary-pressed: "#155bad"
  primary-deep: "#104a8f"
  on-primary: "#ffffff"
  brand-navy: "#0a1530"
  ...
  brand-blue: "#1e73d8"
  brand-blue-300: "#a9e4ff"
  brand-blue-800: "#0f376a"
  brand-teal: "#2a9d99"
  brand-green: "#1aae39"
  brand-yellow: "#f5d75e"
  brand-brown: "#523410"
...
components:
  ...
  card-feature-sky:
    ...
    rounded: "{rounded.lg}"
    padding: "{spacing.xxl}"
  card-feature-sky-soft:
    backgroundColor: "{colors.card-tint-sky-soft}"
    textColor: "{colors.charcoal}"
    rounded: "{rounded.lg}"
    padding: "{spacing.xxl}"
  card-feature-yellow:
...
- **Signature blue pill** ({colors.primary}) primary CTA — Notion's recognizable "Get Notion free" button color
...
- Pastel feature card palette (peach, rose, mint, sky, sky, yellow) echoing the live product database properties
...
### Brand & Primary
- **Notion Blue** ({colors.primary}): Signature primary CTA color — the unmistakable "Get Notion free" pill button. Reserved for the dominant CTA only.
...
- **Brand Blue** ({colors.brand-blue}): Blue accent variant
- **Brand Blue 300** ({colors.brand-blue-300}): Light blue
- **Brand Blue 800** ({colors.brand-blue-800}): Deep blue for tag text
...
- **Tint Sky** ({colors.card-tint-sky-soft}): Pale sky
...
**`button-primary`** — Signature blue rectangular primary CTA, the dominant action.
...
**`button-link`** — Inline blue text link (NOT primary blue).
...
**`card-feature-peach`** + **`card-feature-rose`** + **`card-feature-mint`** + **`card-feature-sky`** + **`card-feature-sky-soft`** + **`card-feature-yellow`** + **`card-feature-cream`** — Pastel-tinted feature cards.
...
**`pricing-card-featured`** — Featured pricing tier (Plus or Business — blue-bordered).
...
- Border switches to `2px solid {colors.primary}` (blue).
...
**`badge-blue`** — Blue status badge (matches primary CTA).
...
**`badge-tag-blue`** — Soft-blue feature tag chip.
- Background `{colors.card-tint-sky-soft}`, text `{colors.brand-blue-800}`, typography `{typography.caption-bold}`, rounded `{rounded.sm}`, padding `2px 8px`.
...
- Right: "Get Notion free" blue button + "Log in" link.
...
- Layout: centered headline `{typography.hero-display}`, subtitle, button row (`button-primary` blue + `button-secondary-on-dark`), `workspace-mockup-card` below.
...
### Do
- Use `{colors.primary}` (blue) as the dominant CTA across all surfaces — it's the brand's recognizable signal
- Pair deep navy hero bands ({colors.brand-navy}) with the blue button + decorative sticky-note dots
- Use pastel feature card tints (peach, rose, mint, sky, sky, yellow) generously
...
### Don't
- Don't use the blue for body text or large background surfaces
- Don't use pill-shaped buttons; Notion's geometry is rectangular-sober
- Don't mix link-blue ({colors.link-blue}) with primary-blue ({colors.primary}) — they have distinct roles
...
6. Keep `{colors.primary}` (blue) as the primary CTA — distinct from `{colors.link-blue}` for inline links
