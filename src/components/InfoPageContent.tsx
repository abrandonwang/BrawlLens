...
function DisclosureGroup({ children }: { children: ReactNode }) {
  return <div className="my-5 border-t border-[var(--line)]">{children}</div>
}
...
export function AboutContent() {
  ...
  useEffect(() => {
    const ids = ["introduction", ...sidebar.flatMap(g => g.links.map(([, href]) => href.replace(/^#/, "")))]
    const elements = ids
  ...
  return (
    <main className="mx-auto w-full max-w-[1200px] px-6 pb-24 max-md:px-4">
      <div className="grid gap-10 pt-10 md:grid-cols-[210px_minmax(0,1fr)] lg:grid-cols-[210px_minmax(0,1fr)_210px] lg:gap-12">
        <aside className="hidden md:block">
          <div className="sticky top-20 flex flex-col gap-4 pb-6">
            {sidebar.map(group => (
              <div key={group.title} className="flex flex-col gap-1">
                <button
                  type="button"
                  onClick={() => toggleGroup(group.title)}
                  className="flex w-full items-center gap-2 border-0 bg-transparent p-0 text-left font-inherit"
                  aria-expanded={openGroups[group.title]}
                >
                  <h4 className="text-[13px] font-semibold tracking-[-0.01em] text-[var(--ink)]">{group.title}</h4>
                  <span className="ml-auto grid size-5 place-items-center text-[var(--ink-3)]">
                    <ChevronRight className={`size-[0.8rem] transition-transform ${openGroups[group.title] ? "rotate-90" : ""}`} />
                  </span>
                </button>
                {openGroups[group.title] && (
                  <div className="mt-1.5 flex flex-col items-start gap-2 border-l border-[var(--line)] pl-3 text-[13px]">
        ...
        <div className="min-w-0">
          <nav aria-label="breadcrumb" className="mb-6">
            <ol className="flex flex-wrap items-center gap-1.5 break-words text-[13px] text-[var(--ink-4)] sm:gap-2.5">
          ...
          <article className="docs-prose w-full max-w-[680px]">
            <h1 id="introduction" className="scroll-mt-20">About BrawlLens</h1>
            <p className="-mt-1 text-[15px] leading-[1.5] text-[var(--ink-3)]">
            ...
            <DisclosureGroup>
              ...
              <Disclosure title="Brawlers">
                <p>The brawler catalog joins static metadata with aggregate battle performance. The page is meant to answer both &ldquo;what is this brawler?&rdquo; and &ldquo;how is this brawler doing?&rdquo;</p>
                <ul>
            ...
            <h2 id="metric-notes">Metric Notes</h2>
            <p>
            ...
            <h2 id="search-help">Search Help</h2>
            <p>
            ...
            <div className="mt-12 flex items-center justify-between border-t border-[var(--line)] pt-6">
              <a className="flex items-center gap-2 text-sm text-[var(--ink-3)] no-underline hover:text-[var(--ink)]" href="#introduction">
                <ChevronRight className="size-[1rem] rotate-180" />
                <span>Back to top</span>
              </a>
              <Link className="flex items-center gap-2 text-sm text-[var(--ink-3)] no-underline hover:text-[var(--ink)]" href="/brawlers">
                <span>Open Brawlers</span>
                <ChevronRight className="size-[1rem]" />
              </Link>
            </div>
          </article>
        </div>
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-20 flex flex-col gap-3 pb-6">
            <h3 className="text-[13px] font-semibold tracking-[-0.01em] text-[var(--ink)]">On this page</h3>
            <div className="flex flex-col gap-2 border-l border-[var(--line)] pl-3 text-[13px]">
              {toc.map(([label, href]) => {
                const id = href.replace(/^#/, "")
                const active = activeSection === id
                return (
