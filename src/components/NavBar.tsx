...
export default function NavBar() {
  ...
  const isTag =
    query.trim().startsWith("#") || /^[A-Z0-9]{3,}$/i.test(query.trim());
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 px-4 border-b border-black/8 dark:border-white/8">
              <Search
                size={15}
                className="text-zinc-400 shrink-0 dark:text-white/50"
              />
              <input
                ref={inputRef}
                value={query}
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && isTag && handlePlayerSearch()
                }
            ...
            <div className="py-2">
              {query && isTag && (
                ...
                >
                  ...
                  <span className="flex-1 text-left">
                    Search player{" "}
                    <span className="text-red-500 dark:text-[#FFD400] font-bold">
                      #{query.replace(/^#/, "")}
                    </span>
                  </span>
                  <ArrowRight
                    size={13}
                    className="text-zinc-300 dark:text-white/40"
                  />
                </button>
              )}
              ...
              {filtered.length === 0 && !isTag && (
                <p className="px-4 py-6 text-xs text-zinc-400 text-center dark:text-white/45">
